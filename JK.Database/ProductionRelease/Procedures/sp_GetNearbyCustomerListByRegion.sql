IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetNearbyCustomerListByRegion]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
  DROP PROCEDURE [dbo].[sp_GetNearbyCustomerListByRegion]
GO
...
CREATE PROCEDURE [dbo].[sp_GetNearbyCustomerListByRegion]
(
  @RegionId INT = 0,
