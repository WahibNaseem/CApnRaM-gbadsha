IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetCustomerListByFranchisee]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
  DROP PROCEDURE [dbo].[sp_GetCustomerListByFranchisee]
GO
...
CREATE PROCEDURE [dbo].[sp_GetCustomerListByFranchisee]
(
  @FranchiseeId INT = 0,
  @PageNo INT = 1,
  @PageSize INT = 10,
...
BEGIN TRY
  ...
  WHERE
  	Distribution.FranchiseeId = @FranchiseeId
  ORDER BY
