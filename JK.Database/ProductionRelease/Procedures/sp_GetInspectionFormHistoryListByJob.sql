IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetInspectionFormHistoryListByJob]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[sp_GetInspectionFormHistoryListByJob]
GO  

CREATE PROCEDURE [dbo].[sp_GetInspectionFormHistoryListByJob]      
(
	@JobId INT = 0,
	@IsEnable BIT = 1,  
	@PageNo INT = 1,  
...
BEGIN TRY
	...
	WHERE
		InspectionFormHistory.JobId = @JobId AND
