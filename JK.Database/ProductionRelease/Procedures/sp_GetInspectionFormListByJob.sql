IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetInspectionFormListByJob]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
    DROP PROCEDURE [dbo].[sp_GetInspectionFormListByJob]
GO  

CREATE PROCEDURE [dbo].[sp_GetInspectionFormListByJob]      
(
    @JobId INT = 0,
    @IsEnable BIT = 1,  
    @PageNo INT = 1,  
...
BEGIN TRY
    ...
    WHERE
        InspectionForm.JobId = @JobId AND
