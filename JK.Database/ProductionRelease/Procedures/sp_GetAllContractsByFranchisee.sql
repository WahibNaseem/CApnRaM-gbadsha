IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetAllContractsByFranchisee]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
  DROP PROCEDURE [dbo].[sp_GetAllContractsByFranchisee]
GO
...
CREATE PROCEDURE [dbo].[sp_GetAllContractsByFranchisee]
(
  @FranchiseeId INT = 0
)
AS
...
BEGIN TRY
  ...
  CREATE TABLE #Temp_Table_ContractsByFranchisee
  (
  ...
  INSERT INTO #Temp_Table_ContractsByFranchisee
    SELECT
      Contract.ContractId,
      Customer.RegionId,
      Customer.CustomerId,
    ...
    SELECT
      ...
      Address.Longitude
    FROM
      #Temp_Table_ContractsByFranchisee ResultTable WITH (NOLOCK)
    ...
    WHERE
      Franchisee.FranchiseeId = @FranchiseeId

  -- Remove the temp tables
  DROP TABLE #Temp_Table_ContractsByFranchisee
