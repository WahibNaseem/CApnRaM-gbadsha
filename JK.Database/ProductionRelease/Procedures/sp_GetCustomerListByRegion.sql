IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetCustomerListByRegion]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
  DROP PROCEDURE [dbo].[sp_GetCustomerListByRegion]
GO
...
CREATE PROCEDURE [dbo].[sp_GetCustomerListByRegion]
(
  @RegionId INT = 0,
  @PageNo INT = 1,
  @PageSize INT = 10,
