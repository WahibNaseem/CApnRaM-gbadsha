IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[portal_FOM_GetFranchiseeByNo]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[portal_FOM_GetFranchiseeByNo]
GO
...
CREATE PROCEDURE [dbo].[portal_FOM_GetFranchiseeByNo]
(
	@FranchiseeNo VARCHAR(10) = 0
)
AS
...
BEGIN TRY
	...
	INSERT INTO #Temp_Table_FranchiseeByFranchiseeId
		...
		WHERE
			...
			Franchisee.FranchiseeNo = @FranchiseeNo AND
		...
		LEFT JOIN [dbo].[Phone] Phone WITH (NOLOCK) ON Phone.ClassId = ResultTable.FranchiseeId AND Phone.TypeListId = 2 AND Phone.ContactTypeListId = 1
