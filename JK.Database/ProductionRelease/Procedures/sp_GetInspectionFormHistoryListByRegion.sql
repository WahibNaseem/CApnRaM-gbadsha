IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetInspectionFormHistoryListByRegion]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[sp_GetInspectionFormHistoryListByRegion]
GO  

CREATE PROCEDURE [dbo].[sp_GetInspectionFormHistoryListByRegion]      
(
	@RegionId INT = 0,
	@IsEnable BIT = 1,  
	@PageNo INT = 1,  
...
BEGIN TRY
	...
	WHERE
		InspectionFormHistory.RegionId = @RegionId
	ORDER BY
