IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetJobListByFranchisee]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[sp_GetJobListByFranchisee]
GO 
...
CREATE PROCEDURE [dbo].[sp_GetJobListByFranchisee]  
(
	@FranchiseeId INT = 0,
	@IsEnable BIT = 1,
...
BEGIN TRY
	...
	WHERE
		Job.IsEnable = @IsEnable AND
		Job.FranchiseeId = @FranchiseeId
	ORDER BY
