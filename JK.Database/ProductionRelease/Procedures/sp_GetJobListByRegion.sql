IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetJobListByRegion]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[sp_GetJobListByRegion]
GO
...
CREATE PROCEDURE [dbo].[sp_GetJobListByRegion]
(
	@RegionId INT = 0,
	@IsEnable BIT = 1,
