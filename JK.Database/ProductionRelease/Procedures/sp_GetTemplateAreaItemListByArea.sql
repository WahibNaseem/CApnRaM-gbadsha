IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetTemplateAreaItemListByArea]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[sp_GetTemplateAreaItemListByArea]
GO  
  
CREATE PROCEDURE [dbo].[sp_GetTemplateAreaItemListByArea]    
(
