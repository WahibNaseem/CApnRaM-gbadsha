IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetInspectionFormHistoryListByCustomer]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[sp_GetInspectionFormHistoryListByCustomer]
GO  

CREATE PROCEDURE [dbo].[sp_GetInspectionFormHistoryListByCustomer]      
(
	@CustomerId INT = 0,
	@IsEnable BIT = 1,  
	@PageNo INT = 1,  
