IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetInspectionFormListByCustomer]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
    DROP PROCEDURE [dbo].[sp_GetInspectionFormListByCustomer]
GO  

CREATE PROCEDURE [dbo].[sp_GetInspectionFormListByCustomer]      
(
    @CustomerId INT = 0,
    @IsEnable BIT = 1,  
    @PageNo INT = 1,  
...
BEGIN TRY
    ...
    WHERE
        InspectionForm.CustomerId = @CustomerId AND
