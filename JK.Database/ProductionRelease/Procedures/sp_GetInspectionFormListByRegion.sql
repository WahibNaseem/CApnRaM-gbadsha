IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetInspectionFormListByRegion]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
    DROP PROCEDURE [dbo].[sp_GetInspectionFormListByRegion]
GO  

CREATE PROCEDURE [dbo].[sp_GetInspectionFormListByRegion]      
(
    @RegionId INT = 0,
    @IsEnable BIT = 1,  
    @PageNo INT = 1,  
...
BEGIN TRY
    ...
    WHERE
        InspectionForm.RegionId = @RegionId
    ORDER BY
