IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetCustomer]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
...
BEGIN TRY
  ...
  CREATE TABLE #Temp_Table_CustomersByCustomerId
  (
  ...
  INSERT INTO #Temp_Table_CustomersByCustomerId
    SELECT
      Customer.RegionId,
      Customer.CustomerId,
      Customer.CustomerNo,
  ...
  SELECT
    ...
    Address.Longitude
  FROM
    #Temp_Table_CustomersByCustomerId ResultTable WITH (NOLOCK)
  ...
  -- Remove temp tables
  DROP TABLE #Temp_Table_CustomersByCustomerId
