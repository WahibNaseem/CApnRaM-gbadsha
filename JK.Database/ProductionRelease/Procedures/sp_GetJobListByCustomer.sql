IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetJobListByCustomer]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[sp_GetJobListByCustomer]
GO 
...
CREATE PROCEDURE [dbo].[sp_GetJobListByCustomer]  
(
	@CustomerId INT = 0,
	@IsEnable BIT = 1,
