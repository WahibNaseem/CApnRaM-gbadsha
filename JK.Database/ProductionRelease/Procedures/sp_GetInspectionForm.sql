IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetInspectionForm]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[sp_GetInspectionForm]
GO  

CREATE PROCEDURE [dbo].[sp_GetInspectionForm]      
(
...
BEGIN TRY
	...
	LEFT JOIN [dbo].[Franchisee] Franchisee WITH (NOLOCK) on Franchisee.FranchiseeId = InspectionForm.FranchiseeId
	...
	WHERE
		InspectionForm.InspectionFormId = @InspectionFormId AND
