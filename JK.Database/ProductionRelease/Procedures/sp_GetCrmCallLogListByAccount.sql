IF EXISTS (SELECT * FROM sysobjects WITH (NOLOCK) WHERE id = object_id(N'[dbo].[sp_GetCrmCallLogListByAccount]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[sp_GetCrmCallLogListByAccount]
GO  

CREATE PROCEDURE [dbo].[sp_GetCrmCallLogListByAccount]      
(
	@AccountId INT = 0,
	@PageNo INT = 1,  
	@PageSize INT = 10,  
...
BEGIN TRY
	...
	WHERE
		CallLog.CRM_AccountId = @AccountId
	ORDER BY
